...
import { ArrowRight, CalendarPlus, FilePenLine, FolderPlus, Users } from 'lucide-react'
...
export function AdminDashboard({ admin, summary, activity }: { admin: CurrentAdmin; summary: DashboardSummary; activity: { id: string; action: string; entity_type: string; entity_id: string; created_at: string }[] }) {
  const actions = [
    ...
    ...(admin.role === 'EDITOR' ? [] : [{ title: 'Manage project teams', detail: 'Add members, pick leads, start projects.', href: '/admin/project-teams', scope: 'projects', icon: Users }]),
  ...
  return <main className="admin-panel portal-home"><div className="admin-page-heading"><div><p className="eyebrow">Club administration</p><h1>What needs doing?</h1></div></div>
    ...
    <section className="portal-section" aria-label="Club status"><div className="portal-summary-links"><Link href={admin.role === 'EDITOR' ? '/admin/projects' : '/admin/project-teams?filter=underway'}><strong>{summary.activeProjects}</strong><span>Active projects</span><ArrowRight size={17}/></Link><Link href="/admin/events"><strong>{summary.upcomingEvents}</strong><span>Upcoming events</span><ArrowRight size={17}/></Link><Link href="/admin/opportunities"><strong>{summary.closingOpportunities}</strong><span>Deadlines in the next 14 days</span><ArrowRight size={17}/></Link></div></section>
