...
import { progressLine } from './workspace/ProjectProgressList'
import type { ClubTeam } from '@/lib/teams/types'

export function MemberDashboard({ displayName, summary, teams, clubTeams = [], progress = [] }: { displayName: string; summary: MemberDashboardSummary; teams: WorkspaceSummary[]; clubTeams?: ClubTeam[]; progress?: ProjectOverview[] }) {
  const progressFor = (projectId: string) => progress.find(item => item.projectId === projectId)
  return <main className="admin-panel portal-home">
    ...
    <section className="portal-section" aria-labelledby="member-teams"><div className="portal-section-heading"><h2 id="member-teams">My teams and projects <span>{teams.length + clubTeams.length}</span></h2><Link href="/member/teams">All teams <ArrowRight size={16}/></Link></div>
      ...
      {teams.length ? <div className="portal-link-list">{teams.slice(0, 4).map(team => <Link key={team.projectId} href={`/member/teams/${team.projectId}`}><FolderKanban size={21}/><span><strong>{team.title}</strong><small>{team.membershipRole === 'LEAD' ? 'Project lead' : 'Team member'} · {progressFor(team.projectId) ? progressLine(progressFor(team.projectId)!) : team.projectStatus.replaceAll('_', ' ')}</small></span><ArrowRight size={19}/></Link>)}</div> : !clubTeams.length && <div className="portal-empty"><FolderKanban size={24}/><div><h3>No team yet</h3><p>Create a team or find one to join.</p></div></div>}
    </section>
