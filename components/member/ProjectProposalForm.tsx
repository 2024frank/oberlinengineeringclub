...
export function ProjectProposalForm({ teams = [], initialTeamId = '' }: { teams?: { id: string; name: string }[]; initialTeamId?: string }) {
  ...
  return <form className="portal-form" onSubmit={submit}><fieldset className="portal-editor-fields" disabled={!ready || busy}>
    ...
    {step === 0 && <><label>Project title<input {...field('title')} minLength={3} maxLength={160} required/></label><label>What problem are you solving?<textarea {...field('problem')} rows={3} minLength={10} maxLength={3000} required/></label><label>What should the project accomplish?<textarea {...field('goal')} rows={3} minLength={10} maxLength={3000} required/></label></>}
    {step === 1 && <>{teams.length > 0 && <label>Propose for<select value={teamId} onChange={event => setTeamId(event.target.value)}><option value="">My own idea</option>{teams.map(team => <option value={team.id} key={team.id}>{team.name}</option>)}</select></label>}<p className="portal-muted">These details are optional.</p><label>Short summary<textarea {...field('summary')} rows={2} maxLength={700}/></label><label>Engineering disciplines <small>Separated by commas</small><input {...field('disciplines')} placeholder="Electrical, Mechanical, Computer Science"/></label><label>Who or what skills do you want to recruit?<textarea {...field('recruitingNeeds')} rows={3} maxLength={1200}/></label><label>Supporting links <small>One URL per line</small><textarea {...field('links')} rows={2}/></label></>}
