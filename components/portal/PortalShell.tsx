  ...
  return <div className={`portal-shell portal-shell--${portal}`}>
    ...
    <div className="portal-workspace">
      <header className="portal-topbar">
        ...
        <div className="portal-topbar-actions">{portal === 'member' && <Link className="portal-icon-button" href="/member/notifications" aria-label={unreadNotifications ? `Notifications, ${unreadNotifications} unread` : 'Notifications'} title="Notifications"><Bell size={19}/>{unreadNotifications > 0 && <span className="portal-badge" aria-hidden="true">{unreadNotifications > 99 ? '99+' : unreadNotifications}</span>}</Link>}<a className="portal-public-link" href={process.env.NEXT_PUBLIC_SITE_URL ?? 'https://oberlin32engineeringsociety.com/'} target="_blank" rel="noreferrer" aria-label="View club website" title="Club website"><span>Club website</span><ArrowUpRight size={17}/></a><SignOutButton portal={portal}/></div>
      </header>
