...
export type ProjectCardData={id:string;slug:string;title:string;summary:string;status:string;difficulty?:string;disciplines:string[];image?:{url:string;alt:string}}
export function ProjectCards({projects,searchable=false}:{projects:ProjectCardData[];searchable?:boolean}) {
  ...
  <div className="project-grid">{filtered.map(p=><Link className="project-tile" href={'/projects/'+p.slug} key={p.id}>{p.image&&<div className="project-tile__image"><Image src={p.image.url} alt={p.image.alt||p.title} fill sizes="(max-width:600px) 90vw,(max-width:1000px) 44vw,390px"/></div>}<div className="project-tile__body"><span className="project-discipline">{[...p.disciplines,p.difficulty].filter(Boolean).join(' / ')}</span><h3>{p.title}</h3><p>{p.summary}</p></div><div className="project-tile__footer"><span className="project-state">{p.status.replaceAll('_',' ').toLowerCase()}</span><ArrowUpRight size={21} aria-hidden="true"/></div></Link>)}</div>
