...
export type DashboardSummary={newSubmissions:number;drafts:number;upcomingEvents:number;activeProjects:number;closingOpportunities:number;scheduledPublications:number;pendingMemberApprovals:number;activeStaffInvites:number;pendingProjectProposals:number;pendingProjectUpdateReviews:number;pendingProjectApplications:number}
export async function getDashboardSummary(role:AdminRole):Promise<DashboardSummary>{const s=await createSupabaseServerClient();const now=new Date();const soon=new Date(now.getTime()+14*86400000).toISOString().slice(0,10);const community=role!=='EDITOR';const[subs,drafts,pageDrafts,events,projects,opps,scheduled,members,staffInvites,proposals,updateReviews,applications]=await Promise.all([
...
]);return{newSubmissions:subs.count??0,drafts:(drafts.count??0)+(pageDrafts.count??0),upcomingEvents:events.count??0,activeProjects:projects.count??0,closingOpportunities:opps.count??0,scheduledPublications:scheduled.count??0,pendingMemberApprovals:members.count??0,activeStaffInvites:staffInvites.count??0,pendingProjectProposals:proposals.count??0,pendingProjectUpdateReviews:updateReviews.count??0,pendingProjectApplications:applications.count??0}}
