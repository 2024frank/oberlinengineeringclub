...
export function projectTeamWelcomeEmail(input:{memberName:string;projectTitle:string;role:'LEAD'|'MEMBER';actionUrl:string}):TransactionalEmailMessage{const lead=input.role==='LEAD';return{subject:lead?`You are a project lead: ${input.projectTitle}`:`You're on the ${input.projectTitle} team`,text:`${hello(input.memberName)}\n\n${lead?`You are now a lead of the OEC project “${input.projectTitle}”. You can invite teammates, review applications, and plan milestones.`:`You are now on the team for the OEC project “${input.projectTitle}”.`}\n\nIn your project workspace you can see who is on the team, pick up milestones, post progress updates, and share links with your teammates.\n\nOpen your project workspace: ${input.actionUrl}${signature}`}}
export function projectKickoffEmail(input:{memberName:string;projectTitle:string;role:'LEAD'|'MEMBER';message?:string;meetingAt?:string|null;location?:string;actionUrl:string}):TransactionalEmailMessage{const when=input.meetingAt?new Date(input.meetingAt).toLocaleString('en-US',{timeZone:'America/New_York',weekday:'long',month:'long',day:'numeric',hour:'numeric',minute:'2-digit'})+' (Eastern)':'';const meeting=when||input.location?.trim()?`\n\nKickoff meeting${when?`\nWhen: ${when}`:''}${input.location?.trim()?`\nWhere: ${input.location.trim()}`:''}`:'';return{subject:`Time to start: ${input.projectTitle}`,text:`${hello(input.memberName)}\n\nThe club officers have started the OEC project “${input.projectTitle}”, and you are on the team${input.role==='LEAD'?' as a project lead':''}.${input.message?.trim()?`\n\n${input.message.trim()}`:''}${meeting}\n\nOpen the project workspace to see the plan, claim a milestone, and post your first update:\n${input.actionUrl}${signature}`}}
